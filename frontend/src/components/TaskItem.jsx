...
const TaskItem = ({task ,onRefresh, onLogout, showCompleteCheckbox=true}) => {
  ...
  return (
    ...
    <div className={`${TI_CLASSES.wrapper} ${borderColor} dark:bg-gray-800 dark:border-gray-700`}>
      <div className={TI_CLASSES.leftContainer}>
        {showCompleteCheckbox && (
          <button onClick={handleComplete}
          className={`${TI_CLASSES.completeBtn} ${isCompleted ? 'text-green-500' : 'text-gray-300 dark:text-gray-600'}`}>
      ...
      <div className=' flex-1 min-w-0'>
        <div className='flex items-baseline gap-2 mb-1 flex-wrap'>
          ...
          ${isCompleted ? 'text-gray-400 dark:text-gray-500 line-through': 'text-gray-800 dark:text-gray-200'}`}>
            {task.title}
          </h3>
          <span className={`${TI_CLASSES.priorityBadge}
          ${getPriorityBadgeColor(task.priority)} dark:bg-opacity-20`}>
            {task.priority}
          </span>
          </div> 
          {task.description && <p className={`${TI_CLASSES.description} dark:text-gray-400`}>
            {task.description}</p>}
      </div>
    </div>
      <div className={TI_CLASSES.rightContainer}>
        <div className='relative'>
          <button onClick={()=> setShowMenu(!showMenu)}
            className={`${TI_CLASSES.menuButton} dark:text-gray-400 dark:hover:text-gray-300`}>
          ...
          {showMenu && (
            <div className={`${TI_CLASSES.menuDropdown} dark:bg-gray-800 dark:border-gray-700`}>
              {MENU_OPTIONS.map(opt=> (
                ...
                className='w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm hover:bg-purple-50 dark:hover:bg-gray-700
                flex items-center gap-2 transition-colors duration-200 cursor-pointer text-gray-700 dark:text-gray-300'>
                  {opt.icon}{opt.label}
        ...
        <div>
          <div className={`${TI_CLASSES.dateRow} ${task.dueDate && isToday(new Date(task.dueDate))
            ? 'text-fuchsia-600 dark:text-fuchsia-400' : 'text-gray-500 dark:text-gray-400'}`}>
              <Calendar className='w-3.5 h-3.5'/>
              {task.dueDate ? (isToday(new Date(task.dueDate)) ?
                'Today' : format(new Date(task.dueDate), 'MMM dd')): '-'}
          </div>
          
          <div className={`${TI_CLASSES.createdRow} dark:text-gray-400`}>
            <Clock className='w-3 h-3 sm:w-3 sm:h-3.5'/>
            {task.createdAt ?
            `Oluşturulma Tarihi: ${format(new Date(task.createdAt), 'MMM dd')}`: 'No date'}
